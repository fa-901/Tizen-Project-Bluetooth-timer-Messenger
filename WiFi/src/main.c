...
static void app_terminate(void *data)
{
    ...
    if (ret != WIFI_ERROR_NONE) {
        PRINT_MSG("[wifi_deinitialize] Failed.");
        dlog_print(DLOG_ERROR, LOG_TAG, "[wifi_deinitialize] Failed.");
    } else {
        PRINT_MSG("[wif_deinitialize] Success.");
        dlog_print(DLOG_ERROR, LOG_TAG, "[wifi_deinitialize] Success.");
    }
}
